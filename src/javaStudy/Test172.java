...
○ 자바의 기본 입출력 개요
   ...
   - 데이터 입출력 시 모든 데이터를 형태와는 관계없이 일련된 흐름으로
     전송을 하는 것이 스트림 입출력 모델의 기본 개념이다.

   - 기본적으로 스트림(stream)에는
     바이트 기반의 스트림과, 문자 기반의 스트림 두 가지가 있다.
...
※ 스트림의 특징
   ...
   - 스트림은 지연될 수 있다.
     스트림에 넣어진 데이터가 처리되기 전에는 스트림에 사용하는 스레드는
	 지연(blocking) 상태에 빠진다.
	 네트워크 상에서는 데이터가 모두 전송되기까지 스레드는 지연(bloking)
	 상태가 된다.
*/
...
public class Test172
{
	public static void main(String[] args) throws IOException
	{
		...
		System.out.println("문자열 입력(종료:Ctrl+z)"); // abcd
		...
		while ((data = System.in.read()) != -1)
		{
			...
			//System.out.print(ch);		//-- 한글 깨짐 (→ 문자 스트림)
			//--==>>
			/*
			문자열 입력(종료:Ctrl+z)
			바보
